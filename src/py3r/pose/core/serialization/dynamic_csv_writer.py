...
def convert_pose_json_to_csv(source_file_path: Path, target_file_path: Path):
    with source_file_path.open("r") as json_file:
    ...
    with StaticPoseCSVWriter(target_file_path, columns) as csv_writer:
        for frame_dict in frame_dicts:
...
class DynamicPoseCSVWriter:
    ...
    def _finalize(self):
        convert_pose_json_to_csv(self.temp_file_path, self.file_path)
