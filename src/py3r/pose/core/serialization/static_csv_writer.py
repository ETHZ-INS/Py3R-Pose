...
class StaticPoseCSVWriter:
    def __init__(self, file_path: Union[Path, str], columns: List[Tuple[str, str, str, str]]):
    ...
    def write_dicts(self, frame_index: int, frame_size: Tuple[int, int], instance_dicts: List[dict]):
        ...
        row: List[Optional[float]] = [frame_index, frame_size[0], frame_size[1]]
    ...
    @staticmethod
    def write_csv(file_path: Union[Path, str], columns: List[Tuple[str, str, str, str]], data: List[Tuple[int, List[dict]]]):
