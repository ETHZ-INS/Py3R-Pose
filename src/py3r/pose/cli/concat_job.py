...
def reset_frame_index():
    ...
    def _reset_frame_index(poses: VideoFramePoses):
        nonlocal frame_index
        poses = replace(poses, frame_index=frame_index)
        frame_index += 1
        return poses

    def _op(upstream: Observable[VideoFramePoses]):
        nonlocal frame_index
        return upstream.pipe(
            ops.map(_reset_frame_index)
        )
...
class ConcatJob:
    def __init__(self):
        ...
        self.reset_frame_index: bool = False
    ...
    def set_reset_frame_index(self, reset_frame_index: bool):
        self.reset_frame_index = reset_frame_index
    ...
    def run(self):
        ...
        poses = rx.concat(*source_observables)

        if self.reset_frame_index:
            poses = poses.pipe(reset_frame_index())
